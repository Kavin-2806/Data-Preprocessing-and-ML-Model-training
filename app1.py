...
st.markdown(
    ...
    * {
    ...
    /* Ensure labels and text in containers are visible */
    [class*="stLabel"],
    [class*="label"],
    ...
    /* Baseweb Dropdown/Select Options Visibility */
    [data-baseweb="menu"],
    [data-baseweb="option"],
    [role="option"],
    [role="listbox"],
    .VirtualizedMenuList,
    .VirtualizedMenuList li,
    [class*="Menu"],
    [class*="Option"] {
        color: #1e293b !important;
    ...
    /* Final catch-all for any hidden text in select/dropdown components */
    .stSelectbox * {
        color: #1e293b !important;
        opacity: 1 !important;
        visibility: visible !important;
    ...
    /* Ensure text in all dropdowns is readable */
    [data-baseweb="select"] * {
    ...
    /* Code and Pre-formatted text visibility */
    pre {
    ...
    /* Code blocks in Streamlit */
    [class*="stCode"] {
        background-color: #1e293b !important;
        color: #e2e8f0 !important;
    }

    /* JSON and dict displays */
    [class*="stText"],
    ...
    /* Expander headers */
    .streamlit-expanderHeader,
    [data-testid="stExpanderToggleButton"] {
        color: #1e293b !important;
        background-color: rgba(255, 255, 255, 0.95) !important;
    }
...
def get_manual_hyperparameters(model_name):
    ...
    if model_name == "Logistic Regression":
        ...
        params["C"] = st.number_input("SVM: C (Regularization)", min_value=0.01, max_value=100.0, value=1.0, step=0.1)
        params["kernel"] = st.selectbox("Kernel", ["linear", "rbf", "poly"], index=1)
        ...
        params["C"] = st.number_input("SVM Regressor: C (Regularization)", min_value=0.01, max_value=100.0, value=1.0, step=0.1)
        params["kernel"] = st.selectbox("Kernel", ["linear", "rbf", "poly"], index=1)
        if params["kernel"] == "poly":
            params["degree"] = st.slider("Polynomial Degree", 2, 5, 3)
        params["gamma"] = st.selectbox("Gamma", ["scale", "auto"], index=0)
